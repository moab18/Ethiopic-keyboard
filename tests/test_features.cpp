...
int main()
{
    ...
    // ── 1. Consonant families: all 7 orders ──
    {
        auto test_order = [&](const std::string &base, const std::string &c6,
            ...
            eng.filter(base); eng.filter("u");
            eng.finish_composition();
            assert_eq(eng.flush(), c2, family + " 2nd '" + base + "u'");

            eng.filter(base); eng.filter("i");
            eng.finish_composition();
            assert_eq(eng.flush(), c3, family + " 3rd '" + base + "i'");

            eng.filter(base); eng.filter("a");
            eng.finish_composition();
    ...
    // ── 2. Labiovelar forms ──
    {
        eng.filter("q"); eng.filter("W"); eng.filter("e");
        eng.finish_composition();
        assert_eq(eng.flush(), "ቈ", "labiovelar 'qWe'");

        eng.filter("k"); eng.filter("W"); eng.filter("a");
        eng.finish_composition();
        assert_eq(eng.flush(), "ኳ", "labiovelar 'kWa'");

        eng.filter("g"); eng.filter("W"); eng.filter("E");
        eng.finish_composition();
    ...
    // ── 9. Punctuation ──
    {
        ...
        eng.filter(":"); eng.filter(":");eng.filter(":");  //eng.filter(" ");
        eng.finish_composition();
